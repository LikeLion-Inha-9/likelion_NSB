...
class Idea_upload(TimeStampModel):
    def number():
        no = Idea_upload.objects.count()
        if no == None:
            return 1
        else:
            return no + 1
    i_id = models.IntegerField(primary_key=True,unique= True,default=number)
...
class Idea_evalu_comment(TimeStampModel):
    def number():
        no = Idea_evalu_comment.objects.count()
        if no == None:
            return 1
        else:
            return no + 1
...
class Service_evalu_comment(TimeStampModel):
    ...
    content = TextField()
    user_id = models.ForeignKey(User,on_delete=models.CASCADE,verbose_name="작성자")
    service_evalu_upload_id = models.ForeignKey(Service_evalu_upload,on_delete=models.CASCADE,verbose_name="Service_upload",related_name="s_evalu_comment")
